...
public final class PatchEnable implements TxCommand {
    ...
    public Result execute(CommandSender executor, String[] args) {
        ...
        if(args.length != 1) {
            return new Result(Status.INVALID_SYNTAX, ChatColor.RED + "Invalid syntax. Usage: /patch enable <internal patch id>");
        }
        
        Optional<TxPatch> patch = PatchManager.getPatchById(args[0]);
        return patch.map(_patch -> {
            if(PatchManager.isEnabled(_patch)) {
                return new Result(Status.ERROR, ChatColor.RED + "This patch is already enabled!");
            }
    ...
    private void sendSuccess(CommandSender executor, TxPatch patch) {
        ...
        executor.sendMessage(String.format(format, ChatColor.GREEN, ChatColor.DARK_GREEN, ChatColor.ITALIC, patch.getMetadata().display_name(), ChatColor.GREEN, ChatColor.DARK_GREEN, ChatColor.GREEN));
    }
